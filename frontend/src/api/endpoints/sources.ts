...
export const sourcesApi = {
  ...
  async analyze(caseId: string, uploadId: string, targetTable?: string): Promise<AnalyzeResult> {
  ...
  async saveMapping(
    caseId: string,
    uploadId: string,
    body: { target_table: string; mapping: Record<string, string>; notes?: string },
  ): Promise<SourceItem> {
  ...
  async import(caseId: string, uploadId: string, body: ImportBody): Promise<TaskRow> {
    const res = await api.post<TaskRow>(
      ...
      { target_table: body.target_table, column_map: body.column_map, clean: body.clean ?? [] },
