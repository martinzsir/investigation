...
// 按命中间类各自计数。纯函数、确定性可测。
import { crossLevel, type CrossLevel } from './clue'

/** 一条交叉信号：命中某间（资金/通讯/行为/关系/时间），来自某个数据源 */
export interface CrossSignal {
  /** 间类名（JIAN_ROOMS 之一；未知间类不计入） */
  room: string
...
const KNOWN_ROOMS = new Set(['资金', '通讯', '行为', '关系', '时间'])

/**
 * 独立间数（FE-T-007）：
 * - 结构化信号：按命中间类去重计数（同一间多个结构化源只算 1 间）；
 * - LLM 信号：同一模型（llmModel 缺失时归入 'llm' 同源桶）的多次判读，
 *   无论判读几间、换什么 prompt（备注/聊天/复算），至多贡献 1 个独立间；
 *   且只在该模型声称的间未被结构化证据覆盖时才占额。
 * 红线用例：LLM 判「备注可疑→资金」+ LLM 判「聊天可疑→通讯」≠ 双源，n=1。
 */
export function countIndependentJians(signals: CrossSignal[]): number {
  const structuredRooms = new Set<string>()
  const llmClaims = new Map<string, Set<string>>()

  for (const s of signals) {
    if (!KNOWN_ROOMS.has(s.room)) continue
    if (s.llmModel) {
...
/** 信号 → 交叉等级（FE-T-006/007：n>=3 候选 / n==2 线索 / 否则观察） */
export function crossLevelFromSignals(signals: CrossSignal[]): CrossLevel {
  return crossLevel(countIndependentJians(signals))
}
