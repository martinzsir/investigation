...
> 版本 **v1.3** ｜ 日期 2026-09-11
> 用户决策：① 质量需要前移，且后续过程中也可验证；② ETL 应在接入过程中发挥作用，明显质量问题或不符合数据元要求的可在界面执行 ETL 操作，尚不支持的提出建议；③ 数据画像在接入过程中未起作用；④ **数据元推荐位置太晚**；⑤ **数据元应是全域的，行业按金融/医疗划分**；⑥ **统一采样规模 200 行**；⑦ **de_reco 草稿与 etl 处置草稿分两张表**
> 依据：`investigation_v3` 全链路核查（ingest 三接口、source_analyze、quality 四扫描、etl 路由、clean_ops 11 op、列画像双实现、analyze 与 worker/recommend 重复计算、**双包数据元隔离实测**、**采样规模不一致实测 200 vs 50**、**草稿区语义混用实测**）
...
> **v1.3 变更：统一采样规模到 200 行（Step1 analyze 与 Step4 worker/recommend 对齐）+ 双草稿表设计（de_recommendation 数据元裁决 / etl_fix_draft ETL 处置 职责分离）**
...
| **v1.3 新增：采样规模** | 🔴 **Step1 analyze 采 200 行、Step4 worker/recommend 采 50 行** —— 同一推荐算法跑两次结果不一致，且异步通道采样偏少导致低置信误判 |
| **v1.3 新增：草稿区** | 🔴 **数据元裁决与 ETL 处置混用一张草稿表** —— 两者生命周期/审批角色/落点全不同，混表导致审计链断裂、状态机错乱 |
...
| 5 | 🔴 **采样规模不一致**（v1.3） | 见 §1.5 |
| 6 | 🔴 **草稿区语义混用**（v1.3） | 见 §1.6 |
...
| 通道 | 文件 | 采样规模 | 代码 |
|---|---|---|---|
| **Step1 analyze**（同步） | `server/app/source_analyze.py:133` | **200 行** | `vals = non_empty.head(200).tolist()` |
| **Step4 worker/recommend**（异步） | `server/app/worker/recommend.py:25` | **50 行** | `_SAMPLE_ROWS = 50` |

#### 为什么是问题

| 危害 | 说明 |
|---|---|
| **结果不一致** | 同一列在 Step1 推荐出数据元 X（采 200 行命中足够），Step4 却推荐为空或另一数据元 Y（采 50 行样本偏少、命中率不足） |
| **低置信误判** | 小样本下 `recommend_for_table` 的 `confidence` 偏低，可能把本应采纳的推荐判为"无推荐" |
| **去重后行为回归风险** | v1.1 删除 Step4 异步任务后，若采样规模未对齐，Step1 结果与历史 Step4 结果不同 → 验收 IN-TC-13（删除后结果一致）无法通过 |
...
_SAMPLE_ROWS = 200  # 原 50，与 source_analyze._column_profile 对齐
```

> 选择 200 的依据：`source_analyze._column_profile` 已验证 200 行采样下 `inferred_type`/`null_rate`/`distinct` 稳定；
> 推荐算法同源同算法，采样规模对齐后结果可逐列比对、可回归验证。

### 1.6 🔴 草稿区语义混用（v1.3 新增）

#### 现状：一张表承载两种不同生命周期

现有 `de_recommendation` 表（`state_store.py:89`）承载"数据元推荐裁决"：
...
若把 ETL 处置草稿也塞进这张表（共用 `recommendations_json` 或加字段），会导致：

| 问题 | 说明 |
|---|---|
| **生命周期不同** | 数据元裁决：Step1→Step2 之间一次性裁决（采纳/驳回即终态）；ETL 处置草稿：Step2→Step3→复核→发布，多次状态流转 |
| **审批角色不同** | 数据元裁决：分析师（clearance≥2）即可；ETL 处置 B 类（会丢信息）：需复核（reviewed_by 不能空） |
| **落点不同** | 数据元裁决 → 写 `bindings.data_element`（映射配置）；ETL 处置 → 写 `bindings.clean`/`bindings.source_sql`（清洗管道） |
| **预演数据不同** | 数据元裁决无预演；ETL 处置 B 类需存 `preview_affected_rows` + `preview_samples_json` |
| **审计链断裂** | 混表后无法区分"这条记录是数据元决策还是 ETL 处置"，审计回溯困难 |

#### 处理方案：双草稿表职责分离

**新增 `etl_fix_draft` 表**，与 `de_recommendation` 表并行，各自管理独立生命周期：

| 表 | 职责 | 生命周期 | 审批 | 落点 |
|---|---|---|---|---|
| `de_recommendation` | **数据元推荐待核实**（列→数据元映射裁决） | Step1→Step2 一次性 | 分析师 clearance≥2 | `bindings.data_element` |
| `etl_fix_draft` | **ETL 处置草稿**（清洗 op 预演+确认） | Step2→Step3→复核→发布 多次流转 | B 类需 reviewed_by | `bindings.clean`/`bindings.source_sql` |
...
Step 1  列分析与映射
        ...
        ↓ 采纳 → 写 de_recommendation（待核实）
        ↓        或直接写 bindings.data_element（见 §6 决策项 6）
Step 2  【新】质量预览与处置   ★ 用 Step1 采纳的数据元作判据（format/enum/clean_rule）
        ↓                     ★ 处置写 etl_fix_draft 草稿（不自动生效）
        ↓                     ★ v1.3：与 de_recommendation 分表
Step 3  确认导入
        ↓ 入队 TASK_IMPORT
Step 4  完成                  ★ v1.1：删除异步推荐任务（去重）
                              ★ v1.3：采样规模统一 200，去重后结果与 Step1 一致
...
| 类 | 判据 | 界面形态 | 落表 | 例子 |
|---|---|---|---|---|
| **A 一键应用** | op 存在 + 无损 | 「应用」按钮 | etl_fix_draft（status=已确认，免复核） | `strip_thousands`、`cn_date_norm`、`despace`、`to_upper` |
| **B 预演 + 确认** | op 存在 + 可能丢信息 | 「预演」→ 显示影响 N 行 → 确认 | etl_fix_draft（status=待复核，需 reviewed_by） | `reject_if`、`digits_only`（账号去横杠） |
| **C 仅建议** | 无 op | 提示上游处理 + 复制建议 | 不落表（仅展示） | **复合列拆分**、跨行去重、语义纠错 |

> **B 类是安全关键**：`reject_if` 会丢行，不能一键应用；必须"预演 → 看影响行数 → 确认"，且草稿需复核才能发布。
> **v1.3 双表分离**：数据元裁决走 `de_recommendation`，ETL 处置走 `etl_fix_draft`，两者审计链独立。
...
| **1-6** | **统一采样规模 200**（v1.3） | `worker/recommend.py` `_SAMPLE_ROWS=200`（若保留通道）/ 或删除后自然对齐 | 0.1 人日 |
...
```python
# server/app/worker/recommend.py
_SAMPLE_ROWS = 200  # 原 50，与 source_analyze._column_profile(.head(200)) 对齐
                    # 保证 Step1 同步推荐与 Step4 异步推荐结果一致
```

> 若 1-5 删除了 Step4 异步任务，1-6 自然满足（采样规模只剩 Step1 一处，即 200）；
> 若保留双通道（Step1 同步 + Step4 异步兜底），1-6 必须显式改 `_SAMPLE_ROWS`。
...
| **2-4** | **建 etl_fix_draft 表 + 配套方法**（v1.3） | 见 §3.7 双草稿表设计 | 0.4 人日 |
...
| 4-3 | 处置 → etl_fix_draft 草稿 → 复核发布（v1.3 双表） | 0.5 人日（复用草稿区机制） |

### 3.7 v1.3 双草稿表设计（关键）

#### 3.7.1 两表对比

| 维度 | `de_recommendation`（已有） | `etl_fix_draft`（新增） |
|---|---|---|
| **语义** | 列→数据元映射推荐，待裁决 | ETL 清洗 op 处置，待预演/复核/发布 |
| **产生时机** | Step1 analyze 后 | Step2 质检发现问题后 |
| **状态机** | 待核实 → 采纳/驳回（终态） | 待复核 → 已确认/已驳回/已发布 |
| **审批角色** | 分析师 clearance≥2 | A 类免复核；B 类需 reviewed_by |
| **落点** | `bindings.data_element` | `bindings.clean` / `bindings.source_sql` |
| **预演数据** | 无 | `preview_affected_rows` + `preview_samples_json` |
| **op 信息** | 无 | `op_token` + `op_class`(A/B/C) |
| **目标** | 列名→数据元 ID | 目标 object + 目标 prop |

#### 3.7.2 etl_fix_draft 表结构
...
CREATE TABLE IF NOT EXISTS etl_fix_draft (
    ...
    upload_id         TEXT NOT NULL,
    target_object     TEXT NOT NULL,      -- 目标 object（如 transaction）
    target_prop       TEXT NOT NULL,      -- 目标属性（如 amount）
    op_token          TEXT NOT NULL,      -- clean op（如 strip_thousands / digits_only）
    op_class          TEXT NOT NULL,      -- A 一键 / B 预演确认 / C 仅建议
    source            TEXT NOT NULL,      -- 来源步骤（Step2 质检 / 手动新增）
    preview_affected_rows INTEGER NOT NULL DEFAULT 0,  -- B 类预演影响行数
    preview_samples_json  TEXT NOT NULL DEFAULT '[]',  -- B 类预演样本（前 N 行前后对比）
    status            TEXT NOT NULL DEFAULT '待复核',  -- 待复核/已确认/已驳回/已发布
    created_at        TEXT NOT NULL,
    created_by        TEXT NOT NULL DEFAULT '',
...
Step2 质检发现问题
   ...
   创建草稿（status=待复核）
       ↓
   ├─ A 类：created_by 直接确认 → status=已确认（免 reviewed_by）
   ├─ B 类：预演 → 填 preview_* → reviewed_by 复核
   │         ├─ 通过 → status=已确认
   │         └─ 驳回 → status=已驳回
   └─ C 类：不落表（仅界面建议）
       ↓
   Step3 确认导入时：
       ├─ status=已确认 → 发布到 bindings.clean/source_sql → status=已发布
       └─ status=待复核/已驳回 → 阻止发布（fail-closed）
```

#### 3.7.4 配套方法（StateStore）

```python
# server/app/store/state_store.py 新增

def save_etl_fix_draft(self, *, draft_id, case_id, upload_id,
                       target_object, target_prop, op_token, op_class,
                       source="Step2", preview_affected_rows=0,
                       preview_samples=None, created_at, created_by,
                       note=""):
    """创建/更新 ETL 处置草稿（幂等：draft_id 存在则覆盖）"""
    ...
    self._conn.execute(
        ...
        [draft_id, case_id, upload_id, target_object, target_prop,
         op_token, op_class, source, preview_affected_rows,
         _json.dumps(preview_samples or [], ensure_ascii=False, default=str),
         created_at, created_by, note])
    self._conn.commit()

def list_etl_fix_drafts(self, *, case_id, upload_id=None, status=None):
    """查询 ETL 处置草稿（可按 upload_id / status 过滤）"""
...
def publish_etl_fix_draft(self, *, draft_id, reviewed_by, reviewed_at):
    """发布 ETL 处置草稿（仅 status=已确认 可发布）"""
    row = self._conn.execute(
        "SELECT status FROM etl_fix_draft WHERE draft_id=?",
        [draft_id]).fetchone()
    if row is None:
        raise KeyError(f"草稿不存在：{draft_id}")
    if row["status"] != "已确认":
        raise ValueError(f"仅已确认草稿可发布，当前状态：{row['status']}")
    self._conn.execute(
        "UPDATE etl_fix_draft SET status='已发布', "
        "reviewed_by=?, reviewed_at=? WHERE draft_id=?",
        [reviewed_by, reviewed_at, draft_id])
    self._conn.commit()
```

### 3.8 v1.1：数据元前移带来的闭环（关键收益）

```
Step1 推荐「手机号 → DE_PHONE」→ 采纳 → 写 bindings.data_element
        ↓
Step2 用 DE_PHONE.format 校验 / 用 DE_PHONE.clean_rule 选 op
        ↓                              ↓ 处置落 etl_fix_draft（v1.3 分表）
...
| 处置 | **界面 ETL 三分类**（落 etl_fix_draft） | 报告 + 跳 ETL 设置 |
| 状态 | 新增 | 已有（保留） |
...
| **一、快赢** | 列画像展示 + 自动质检 + 标注 + **元推荐前移 Step1** − 删除异步任务 + **采样统一 200** | **1.1 人日**（v1.3 +0.1） | 紧随阶段〇 |
| **二、地基** | clean_rule + split op + 扩 op + **etl_fix_draft 建表** | **2.4–3.4 人日**（v1.3 +0.4） | 做完界面才有意义 |
| **三、前移** | 接入态质检新入口 | 2–3 人日 | |
| **四、界面** | 三分类处置 + 预演 + **双草稿表接线** | 2–3 人日 | |

**合计约 8.6–11.6 人日（v1.1 去重 −0.5；v1.2 合入全域化 +1.1；v1.3 采样统一 +0.1 + 双草稿表 +0.4）。**
...
>
> **最小可用组合**：阶段〇（1.1）+ 阶段一（1.1）= **约 2.2 人日**，
> 即可让数据元全域共享、列画像可见、推荐前移且采纳有回写、采样规模一致。
...
| 3 | B 类预演的样本量 | 全量还是前 N 行？影响性能与准确性（v1.3 建议与采样规模一致，取 200 行） |
| 4 | 处置是否需审批 | 当前 `require_analyst`（clearance≥2）够不够；B 类是否需双签（created_by + reviewed_by） |
...
| 7 | **etl_fix_draft 发布后是否需重跑 BUILD？**（v1.3） | 发布即写 bindings.clean，但 bindings 生效需重跑 BUILD；是否在 Step3 确认导入时自动触发 |
...
| IN-TC-06 | `reject_if` 处置 | **先预演显示影响行数**，确认后写 etl_fix_draft（待复核） |
| IN-TC-07 | 处置后 | 生成 etl_fix_draft 草稿，**不自动生效** |
...
| **IN-TC-16** | **采样规模一致**（v1.3） | Step1 analyze 与 Step4 worker/recommend（若保留）推荐结果**逐列一致** |
| **IN-TC-17** | **采样=200**（v1.3） | `_SAMPLE_ROWS=200`，与 `source_analyze._column_profile` 的 `.head(200)` 对齐 |
| **IN-TC-18** | **双草稿表分离**（v1.3） | 数据元裁决落 `de_recommendation`，ETL 处置落 `etl_fix_draft`，**两表无交叉写入** |
| **IN-TC-19** | **A 类免复核**（v1.3） | A 类草稿 created_by 即可确认发布，reviewed_by 可空 |
| **IN-TC-20** | **B 类强制复核**（v1.3） | B 类草稿 reviewed_by 为空时**发布被拒**（fail-closed） |
| **IN-TC-21** | **B 类预演存样**（v1.3） | etl_fix_draft 的 `preview_affected_rows` + `preview_samples_json` 非空 |
| **IN-TC-22** | **未确认不发布**（v1.3） | status=待复核/已驳回 的草稿**无法发布**到 bindings |
...
>
> **v1.3 最关键的判断**：① 采样规模必须统一——Step1 采 200 行、Step4 采 50 行，
> 同一推荐算法跑两次结果不一致，去重后无法通过回归验收；统一 200 即可逐列比对。
> ② 草稿区必须分表——数据元裁决（一次性、分析师、写 bindings.data_element）
> 与 ETL 处置（多次流转、B 类需复核、写 bindings.clean）生命周期/审批/落点全不同，
> 混表导致审计链断裂；双表分离后各自状态机独立、审计链清晰。
>
...
>
> **所有处置都不自动生效**：生成草稿 → 复核 → 发布 → 重跑 BUILD
> （数据元裁决走 `de_recommendation`，ETL 处置走 `etl_fix_draft`，同一条红线，两张表）。
