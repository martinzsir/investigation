...
读：GET /cases/{cid}/review/queue —— needs_review 候选全量（从 obj_org 实时
    构建 OrganizationResolver），排除已裁决候选。
读：GET /cases/{cid}/review/{rid}/evidence —— 候选双方属性对比、差异行、
    相似度依据（merge_reason + evidence）。
...
def _candidates(case_id: str, ctx: WebContext):
    ...
    try:
        ...
        return resolver.review_candidates()
    finally:
        store.close()
...
def review_queue(case_id: str,
    ...
    try:
        cands = _candidates(case_id, ctx)
    except FileNotFoundError:
        cands = []
    decided = _decided_ids(ctx.factory.case_dir(case_id))
    items = [c.to_dict() for c in cands if c.entity_id not in decided]
